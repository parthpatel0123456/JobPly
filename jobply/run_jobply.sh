...
# No Chrome profile; rely on HTTP fetch
...
# Run for 2 minutes, then timeout
timeout 120s python -m discovery_worker.worker 2>&1 | tee /tmp/jobply_run.log
echo "Run finished. Log saved to /tmp/jobply_run.log"